...
function get_login_name($conn, $user_id)
{
	$query = "
	SELECT loginname FROM market_user WHERE user_id = '".$user_id."'
	";
...
function count_total_market_user($conn)
{
	$query = "
	SELECT * FROM market_user WHERE enabled='Yes'";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
}

function fill_product_list($conn)
{
	$query = "
	SELECT * FROM product 
	...
	foreach($result as $row)
	{
		$output .= '<option value="'.$row["code"].'">'.$row["product_name"].'</option>';
	}
	return $output;
}

function fetch_product_details($code, $conn)
{
	$query = "
	SELECT * FROM product 
	WHERE code= '".$code."'";
...
function get_market_by_name_and_location($conn, $market_id)
{
	$query = "
	SELECT market_name, location FROM market WHERE market_id = '".$market_id."'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
...
function count_total_markets($conn)
{
	$query = "
	SELECT * FROM market WHERE market_status='active'";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
}
...
function get_market_user_by_product_by_market($conn)
{
	$query = '
	SELECT mu.fullname, mu.phoneno, p.product_name, p.price, p.quantity, m.market_name, m.location
	FROM market_user mu LEFT JOIN market m ON mu.`user_id` = m.`user_id`
	LEFT JOIN product p ON m.`code` = p.`code`;'
	...
	foreach($result as $row)
	{
		...
		<tr>
			<td>'.$row['fullname'].'</td>
			<td align="right">$ '.$row["phoneno"].'</td>
			<td align="right">$ '.$row["product_name"].'</td>
			<td align="right">$ '.$row["price"].'</td>
			<td align="right">$ '.$row["quantity"].'</td>
			<td align="right">$ '.$row["market"].'</td>
...
function available_product_quantity($connect, $code)
{
	$product_data = fetch_product_details($code, $connect);
	$query = "
